            ...
            function( $post, $data )
            {
              ...
              if( ! empty( $custom_fields ) )
              {
                ...
                foreach( $custom_fields as $label => $type )
                {
                    ...
                    $value = '_' . SoundlushHelpers::uglify( $label );

                    //$meta = get_post_meta( $post->ID, $field_id_name );
                    //var_dump($meta);

                    echo '<label for="' . $field_id_name . '">' . $label . '</label>';
                    echo '<input type="text" name="custom_meta[' . $field_id_name . ']" id="' . $field_id_name . '" value="' . $meta[$value][0] . '" />';
          ...
          if( isset( $_POST ) && isset( $post->ID ) && get_post_type( $post->ID ) == $post_type_name )
          {
            ...
            foreach( $custom_fields as $title => $fields )
            {
                ...
                foreach( $fields as $label => $type )
                {
                    $field_id_name = SoundlushHelpers::uglify( $data['id'] ) . '_' . SoundlushHelpers::uglify( $label );
                    $value = '_' . SoundlushHelpers::uglify( $label );
                    update_post_meta( $post->ID, $value, $_POST['custom_meta[' . $field_id_name . ']'] );
                }
